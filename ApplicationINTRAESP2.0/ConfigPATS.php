 ...
 function SetRow($tab1,$tab2,$tab3,$tab4,$tab5,$tab6)
{
  ...
  for ($i=0; $i <count($tab1) ; $i++) { 
    echo "<tr class=''>"."<td>".utf8_encode($tab1[$i])."</td>"."<td>".utf8_encode($tab2[$i])."</td>"."<td width='12%'>".utf8_encode($tab3[$i])."</td>"."<td width='15%'>".utf8_encode($tab4[$i])."</td>"."<td width='12%'>".utf8_encode($tab5[$i])."</td>"."<td>".utf8_encode($tab6[$i])."</td></tr>";  
  }
}
...
if ($status =="permanent") {
  $req = $bdd->query("SELECT NOM, PRENOMS, MledeSolde, GRADE, FONCTIONS,`DEPT/SERVICE` FROM `agent_pats` WHERE not isnull(MledeSolde) AND MledeSolde <> '_' ") or die(print_r($rep->errorInfo()));
}elseif ($status=="vacataire") {
  // S'il sont vacataires ils n'ont donc pas de matricule pour le moment
  $req = $bdd->query("SELECT NOM, PRENOMS, MledeSolde, GRADE, FONCTIONS,`DEPT/SERVICE` FROM `agent_pats` WHERE  isnull(MledeSolde) OR MledeSolde = '_'  ") or die(print_r($rep->errorInfo()));
}
      ...
      while ($donne = $req->fetch()) {
        ...
        array_push($StructureAgent, $donne['DEPT/SERVICE']);
